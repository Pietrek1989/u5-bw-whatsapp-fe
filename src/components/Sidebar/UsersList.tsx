...
const UsersList: React.FC<{
  ...
  const fetchUsers = async () => {
    try {
      const res = await fetch(
        `${process.env.REACT_APP_BE_URL}/users?q=${query}`,
        {
          headers: {
      ...
      if (res.ok) {
        ...
        setUsers([
          ...(me ? [me] : []),
          ...rest.sort((u1: User, u2: User) => (u1.name < u2.name ? -1 : 1)),
        ]);
      } else {
  ...
  return (
    <div className="users-list">
      ...
      <div
        className="search-field-container bg-white align-items-center justify-content-between"
